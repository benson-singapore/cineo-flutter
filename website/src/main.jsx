    ...
    <div className="min-h-screen overflow-hidden">
      ...
      <main id="top">
        ...
        <section id="features" className="container-wide scroll-mt-28 py-28 sm:py-36">
          ...
          <div className="mt-16 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{features.map((feature, index) => <Card key={feature.number} className={`group overflow-hidden ${index === 1 ? 'md:translate-y-10' : ''}`}><div className="relative aspect-[4/5] overflow-hidden bg-black"><img src={asset(feature.image)} alt={feature.text} className={`h-full w-full object-cover ${index === 2 ? 'object-center' : 'object-top'} opacity-75 transition duration-700 group-hover:scale-[1.02] group-hover:opacity-100`} /><div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-panel via-transparent to-transparent" /><span className="absolute left-5 top-5 font-mono text-xs text-cineo">{feature.number}</span><span className="absolute right-5 top-5 flex h-9 w-9 items-center justify-center rounded-full border border-white/15 bg-ink/60 text-cineo-light backdrop-blur"><feature.icon size={16} /></span></div><div className="p-6"><h3 className="font-display text-2xl font-semibold tracking-tight">{feature.title}</h3><p className="mt-3 text-sm leading-6 text-muted">{feature.text}</p></div></Card>)}</div>
        </section>
